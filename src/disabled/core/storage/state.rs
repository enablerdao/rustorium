use super::{StorageEngine, TypedStorage, CF_METADATA, CF_TRANSACTIONS, PREFIX_META, PREFIX_TX};
...
impl StateManager {
    ...
    pub async fn save_transaction(&self, tx: Transaction) -> Result<()> {
        ...
        TypedStorage::put(&*self.storage, CF_TRANSACTIONS, key, tx).await
    }

    /// トランザクションを取得
    pub async fn get_transaction(&self, tx_id: &TxId) -> Result<Option<Transaction>> {
        let key = self.make_tx_key(tx_id);
        TypedStorage::get(&*self.storage, CF_TRANSACTIONS, key).await
    }
    ...
    pub async fn save_transactions(&self, txs: Vec<Transaction>) -> Result<()> {
        let pairs: Vec<_> = txs
            ...
            .collect();
        TypedStorage::batch_write(&*self.storage, CF_TRANSACTIONS, pairs).await
    }

    /// グローバルステートを更新
    pub async fn update_global_state(&self, state: GlobalState) -> Result<()> {
        let key = self.make_meta_key(b"global_state");
        TypedStorage::put(&*self.storage, CF_METADATA, &key, &state).await
    }

    /// グローバルステートを取得
    pub async fn get_global_state(&self) -> Result<Option<GlobalState>> {
        let key = self.make_meta_key(b"global_state");
        TypedStorage::get(&*self.storage, CF_METADATA, &key).await
    }
    ...
    pub async fn save_metadata<V: Serialize + Send + Sync>(
        ...
        TypedStorage::put(&*self.storage, CF_METADATA, &full_key, &value).await
    }
    ...
    pub async fn get_metadata<V: for<'de> Deserialize<'de> + Send + Sync>(
        ...
        let full_key = self.make_meta_key(key);
        TypedStorage::get(&*self.storage, CF_METADATA, &full_key).await
    }
...
impl StateSyncManager {
    ...
    pub async fn apply_state_diff(&self, diff: StateDiff) -> Result<()> {
        // トランザクションを適用
        self.storage
            .batch_write(CF_TRANSACTIONS, diff.transactions.into_iter().map(|tx| (self.make_tx_key(&tx.id), &tx)).collect())
            .await?;

        // メタデータの変更を適用
        self.storage
            .batch_write(CF_METADATA, diff.metadata_changes.into_iter().map(|(k, v)| (self.make_meta_key(&k), &v)).collect())
            .await?;
